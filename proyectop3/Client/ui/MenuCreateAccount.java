...
public class MenuCreateAccount extends Menu {
	...
	public static void init(Panel root) {
		if (instance == null) {
			...
			createButton = new MenuString(root, "Crear", new Font("Ink Free", Font.BOLD, 30), MenuString.Side.CENTER,
					450);

			notify = new MenuString(root, "", new Font("Ink Free", Font.BOLD, 30), MenuString.Side.L,
					MenuString.Side.L);

			backButton = new MenuString(root, "Atras", new Font("Ink Free", Font.BOLD, 30), MenuString.Side.CENTER,
					625);
	...
	public void mouseAction(int button) {
		...
		if (createButton.isClick(button)) {
			...
			byte resByte[] = Client
					.sendMessage("PUT CREATE " + username.getEnteredString() + " " + password.getEnteredString());
