...
public class MenuMultiPlayerRoomCreate extends Menu {
	...
	public static void init(Panel root) {
		if (instance == null) {
			...
			createButton = new MenuString(root, "Create", new Font("Ink Free", Font.BOLD, 30), MenuString.Side.CENTER,
					600);
	...
	public void mouseAction(int button) {
		...
		if (createButton.isClick(button)) {
			try {
				//clear();
				byte[] buffer = Client.sendMessage(
						"PUT CREATE_BOARD" + " " + widthBoard.getEnteredString() + " " + heightBoard.getEnteredString()
								+ " " + speedBoard.getEnteredString() + " " + appleNumber.getEnteredString());
				String response = new String(buffer).trim();
				if (response.startsWith("Error")) {
					JOptionPane.showMessageDialog(null, "Error: " + response.substring(6));
					return;
				}

				Board.setSizeGrid(widthBoard.getEnteredNumber(), heightBoard.getEnteredNumber());
				Panel.setMenu(MenuMultiPlayer.getInstance());
				MenuMultiPlayer.setStringInfo("ID: " + response);
			} catch (Exception e) {
				e.getMessage();
			}
