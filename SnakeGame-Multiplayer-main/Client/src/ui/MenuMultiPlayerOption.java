...
public class MenuMultiPlayerOption extends Menu {
	...
	public static void init(Panel root) {
		if (instance == null) {
			...
			serverAddress = new MenuEntry(root, "Ingrese la IP: ", "127.0.0.1", new Font("TimesRoman", Font.BOLD, 20),
					300, 350);

			serverPort = new MenuEntry(root, "Ingrese el puerto del server: ", "3333", new Font("TimesRoman", Font.BOLD, 20), 300,
					400);

			connectButton = new MenuString(root, "Conectar", new Font("TimesRoman", Font.BOLD, 30), MenuString.Side.CENTER, 450);
