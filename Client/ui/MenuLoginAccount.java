...
import proyectop3.Client.service.Client;
...
public class MenuLoginAccount extends Menu {
	...
	public static void init(Panel root) {
		if (instance == null) {
			...
			username = new MenuEntry(root, "Ingresa tu nombre de usuario: ", "", new Font("TimesRoman", Font.BOLD, 20),
					300, 350);

			password = new MenuEntry(root, "Ingresa la contraseña: ", "", new Font("TimesRoman", Font.BOLD, 20), 300,
					400);

			createButton = new MenuString(root, "Login", new Font("TimesRoman", Font.BOLD, 30), MenuString.Side.CENTER, 450);
			
			notify = new MenuString(root, "", new Font("TimesRoman", Font.BOLD, 30), MenuString.Side.L, MenuString.Side.L);
			
            backButton = new MenuString(root, "Atras", new Font("TimesRoman", Font.BOLD, 30), MenuString.Side.CENTER, 625);
	...
	public void mouseAction(int button) {
		...
		if (createButton.isClick(button)) {
            ...
            if(res != 0) {
				notify.setText("Ha ocurrido un error");
			}
			else {
                notify.setText("");
